...
describe Bake::Modernize::Gems do
	...
	it "ensures multiple dependencies using a validated transform" do
		content = <<~RUBY
			source "https://rubygems.org"
		RUBY
		updated = <<~RUBY
		...
		mock(Async::Ollama::Transform) do |mock|
			mock.replace(:call) do |input, model:, instruction:, template:|
				expect(input).to be == content
				expect(model).to be == Bake::Modernize::DEFAULT_TRANSFORM_MODEL
				expect(instruction).to be(:include?, "`rubocop`, `rubocop-md`")
				expect(template).to be(:include?, %{gem "rubocop"})
				updated
			end
		end
		
		expect(subject.ensure_dependencies(gems_path, ["rubocop", "rubocop-md"], group: :test, template: updated)).to be_truthy
		expect(File.read(gems_path)).to be == updated
	end
